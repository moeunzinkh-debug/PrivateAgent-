...
- **Copy Text Button (Khmer)** — Every chat bubble has a one-tap copy button labeled in Khmer («ចម្លងអត្ថបទ» = "Copy text"). Tapping it copies the message to the clipboard and briefly confirms with «ចម្លងរួចរាល់» ("Copied").
...
This repository is a copy of [orailnoor/cross-platform-llm-client](https://github.com/orailnoor/cross-platform-llm-client) (PrivateLM) with one addition:

- **New UI button — Copy text (in Khmer)**: a `ចម្លងអត្ថបទ` button is shown under every chat message in `lib/widgets/chat_bubble.dart`. Tapping it copies that message's text to the system clipboard via `Clipboard.setData` and shows a `ចម្លងរួចរាល់` ("Copied") confirmation for 2 seconds. The label renders with the Noto Sans Khmer font.

> **Note:** the prebuilt Stable Diffusion JNI native libraries (`libsd_jni.so`, `libsd_jni_opencl.so`, `libsd_jni_vulkan.so` under `local_plugins/sd_flutter_android/android/src/main/jniLibs/arm64-v8a/`, ~108 MB total) were excluded from this copy because of repository size limits. To build the Android app with local image generation, restore them from the upstream repository or build them from the `stable-diffusion.cpp` submodule.
